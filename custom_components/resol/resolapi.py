...
ResolEndPoint = namedtuple(
    "ResolEndPoint",
    "internal_unique_id, serial, name, friendly_name, value, unit, description, destination, source",
)
...
class ResolAPI:
    ...
    def detect_device(self):
        try:
            ...
            if response.status_code == 200:
                ...
                if matches:
                    self.device = {
                        ...
                        "host": self.host,
                        "port": self.port,
                        "mac": self.format_serial_to_mac(
                            re.search(r'serial\s=\s["](.*?)["]', response.text).group(1)
                        ),  # convert serial to MAC address
                    }
                    _LOGGER.debug(
                        f"{self.device['serial']}: Resol device data received: {self.device}"
                    )
                else:
            ...
            _LOGGER.warning(
                f"Unable to connect to {self.host}. Device might be offline."
            ...
            _LOGGER.error(
                f"Error detecting Resol device - {e}" + ISSUE_URL_ERROR_MESSAGE
            )
            raise IntegrationError(error)
            return None
    ...
    def fetch_data_km2(self):
        ...
        _LOGGER.debug(
            f"{self.device['serial']}: KM2 requesting sensor data from url {url}"
        )
        ...
        try:
            ...
            if "error" in response_auth:
                ...
                _LOGGER.warning(
                    f"{self.device['serial']}: Authentication failed: {error_message}"
                )
            ...
            # GOOD RESPONSE: [{'jsonrpc': '2.0', 'id': '1', 'result': {'authId': 'a463cc3ab307c7ef7d85f22daf15f0'}}]
                ...
                _LOGGER.debug(
                    f"{self.device['serial']}: Successfully authenticated. Auth ID: {authId}"
                )
    ...
    # Fetch the data from the Resol DL2 device, which then constitues the Sensors
    ...
    def __parse_data(self, response):
        ...
        for header in response["headers"]:
            _LOGGER.debug(
                f"{self.device['serial']}: Found header[{iHeader}] now parsing it ..."
            )
            iField = 0
            for field in response["headers"][iHeader]["fields"]:
                ...
                unique_id = (
                    self.device["serial"] + "_" + header["id"] + "__" + field["id"]
                )
                data[unique_id] = ResolEndPoint(
                    internal_unique_id=unique_id,
                    serial=self.device["serial"],
                    name=self.device["serial"].lower()
                    + "_"
